...
class ZoomMouseMap extends RectWidget {
  ...
  public void draw() {
    ...
    PVector translation = new PVector(this.center.x - focus.x, this.center.y - focus.y);
  ...
  private int getXOfCity(City city, PVector translation, PVector focus) {
    float xMin = focus.x - (7 * this.mainMap.rectWidth / 2);
    float xMax = focus.x + (7 * this.mainMap.rectWidth / 2);

    // Mapping de la ville et translation vers la zoomMap
    return int(mapX(city.x, xMin, xMax)) + int(translation.x);
  }

  // Calcul la coordonnée en X de la ville en paramètre sur mainMap avec un zoom de 700% et effectue la translation vers la zoomMap
  private int getYOfCity(City city, PVector translation, PVector focus) {
    float yMin = focus.y - (7 * this.mainMap.rectHeight / 2);
    float yMax = focus.y + (7 * this.mainMap.rectHeight / 2);
    return int(mapY(city.y, yMin, yMax)) + int(translation.y);
  }
}
