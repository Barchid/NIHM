...
public class HMM {
	...
	public void recognize() {
		...
		for (int c = 0; c < gestureClasses.size(); c++) {
			...
			if (this.hasPostProcessing) {
				// Post processing sur le résultat
	...
	/**
	 * Retourne la distance normalisee entre 0 et 1 de la s�quence de points
	 * observ�s
	 * 
	 * @param points
	 * @return
	...
	private double getDistanceNormalisee(Vector<Point2D> points) {
		// r�cup�rer les limites du pattern
...
//		System.out.println(this.cpt);
	...
	public void TestAllExamples() {
		...
		for (int c = 0; c < gestureClasses.size(); c++) {
			...
			for (int i = 0; i < gestClass.getNumberExamples(); i++) {
				...
				// Les raw points à détecter deviennent les
				...
				// SI [la classe trouvée est la bonne]
	...
	public ArrayList<Double> computeFeatures(Vector<Point2D> points) {
		...
		for (int i = 1; i < points.size(); i++) {
			...
			// CALCULER [la feature � base de l'angle entre la droite precedent-courant et
			// l'horizontle
			...
			// Pr�c�dent devient le courant pour le prochain point
			precedent = courant;
		}
	...
	/**
	 * Donne la valeur absolue de l'angle form� par la droite AB et l'horizontal �
	 * 10 degr�s pr�s
	 * 
	 * @param a
	 * @param b
	 * @return valeur absolue de l'angle � 10 degr�s pr�s (de 0 � 180)
	 */
	...
	protected Vector<Point2D> resample(Vector<PointData> pts, int deltaTms) {
		...
		// POUR CHAQUE [point] (ils sont d�j� tri�s par timestamp)
		...
		// Ajout du dernier point (dans le cas o� l'intervalle ne se finit pas dans la
		// boucle)
		Point2D resampledPoint = new Point2D((sumX / nbPtInterval), (sumY / nbPtInterval));
		res.add(resampledPoint);
